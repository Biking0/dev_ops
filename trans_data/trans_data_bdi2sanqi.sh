...
do
    ...
    then 
		...
		beeline -u "jdbc:hive2://ocdpdn80:10000/default" -n ocetl -p demo -e "load data inpath '/user/hive/warehouse/${source_dir}' overwrite into table ${TODIR} partition(${partition_dir})"
        if [ $? -eq 0 ]
        then
