#!/bin/bash
# ***************************************************************************
# 文件名称：trans_data_sanqi2ftp102.sh
# 功能描述：三期传数据到ftp102
...
# 修改日期：
# ***************************************************************************
# 程序调用格式：sh trans_data_sanqi2ftp102_day.sh dm_user_incite_label_yyyymmdd 20200110
...
hive -e "select concat(nvl(statis_time,''),'|'
                      ...
                      ,nvl(city_id,''),'|'
                      ,nvl(id,'')) 
         from $table_name where day_id=$day_id" >${local_data_path}dm_user_incite_label_$day_id.txt;
...
binary
mput ./dm_user_incite_label_$day_id.txt

bye
EOF
#*** 删除本地的冗余文件 
rm /hdfs/data9/port_files/dm_user_incite_label_$day_id.txt
